...
class TelemetryData(BaseModel):
    ...
    parcel_id: str = Field(..., min_length=1, max_length=50, description="Unique parcel identifier")
    device_id: str = Field(..., min_length=1, max_length=100, description="Device identifier")
    temperature: float = Field(..., ge=-100.0, le=200.0, description="Temperature reading in Celsius")
    tilt_x: float = Field(..., ge=-180.0, le=180.0, description="Tilt angle on X-axis in degrees")
    tilt_y: float = Field(..., ge=-180.0, le=180.0, description="Tilt angle on Y-axis in degrees")
    ...
    @classmethod
    def validate_gps_not_zero(cls, v, values, field, **kwargs):
...
def ingest_telemetry():
    ...
    try:
        ...
        # Create document with server-side metadata
        document = {
        ...
        print(f"[INGEST] Successfully ingested telemetry for parcel {telemetry.parcel_id}, "
              f"document_id: {result.inserted_id}")
